...
exports.themhoadon = async (req, res, next) => {
  ...
  if (req.method == "POST") {
    ...
    try {
      ...
      let selectedProduct = listsp.find(product => product._id.toString() === req.body.sanpham);
      selectedProduct.soluong -= req.body.soluong;
      await mydb.spmd.findByIdAndUpdate(selectedProduct._id, { soluong: selectedProduct.soluong });
